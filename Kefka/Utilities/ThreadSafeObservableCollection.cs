...
namespace Kefka.Utilities
{
    ...
    /// thread is a Dispatcher (WPF/Silverlight/WinRT) thread. This means that you can update this from any thread and receive notifications of those updates
    /// on the UI thread.
    ///
    /// You can't modify the collection during a callback (on the thread that received the callback -- other threads can do whatever they want). This is the
    /// same as <see cref="ObservableCollection{T}"/>.
    /// </summary>
    [Serializable, DebuggerDisplay("Count = {Count}")]
    public class ThreadSafeObservableCollection<T> : IList<T>, IReadOnlyList<T>, IList, INotifyCollectionChanged, INotifyPropertyChanged, ISerializable
    {
        // we implement IReadOnlyList<T> because ObservableCollection<T> does, and we want to mostly keep API compatibility...
        ...
        private readonly ReaderWriterLockSlim _lock;                // whenever accessing the collection directly, you must acquire the lock
        ...
        private sealed class ThreadView
        {
            ...
            private readonly ThreadSafeObservableCollection<T> _owner;                     // the collection
            ...
            public ThreadView(ThreadSafeObservableCollection<T> owner)
            {
                _owner = owner;
        ...
        object ICollection.SyncRoot { get { throw new NotSupportedException("TestThreadsafeObservableCollection doesn't need external synchronization"); } }
        ...
        /// </summary>
        public ThreadSafeObservableCollection(SerializationInfo info, StreamingContext context)
